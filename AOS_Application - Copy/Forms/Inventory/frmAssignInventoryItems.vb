...
Public Class frmAssignInventoryItems
    ...
    Private Sub getAvailableInventory(vProductID As Integer)
        ...
        oAvailable.Query.Where(oAvailable.Query.Productid.Equal(vProductID), oAvailable.Query.Inventoryclass.Equal(1), oAvailable.Query.Itemstatus.Equal("AVAILABLE"))
    ...
    Private Sub rbtnAllocateItem_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles rbtnAllocateItem.ItemClick
        ...
        'If oWOItem.Sourcetype <> Nothing Then
        '    MsgBox("The selected work order item has already been assigned a different product source", MsgBoxStyle.Critical, "Error")
        '    Exit Sub
        'End If
        ...
        oInvItem.Allocateddate = Today
        oInvItem.Allocateddocument = oWOItem.Workordernumber
        oInvItem.Itemstatus = "ALLOCATED"
        oInvItem.Save()
    ...
    Private Sub deletePurchaseRequest(vReqKey As Integer)
        ...
        oItems.Query.Where(oItems.Query.Workordernumber.Equal(vWorkOrderID), oItems.Query.Sourcetype.Equal("PREQ"), oItems.Query.Sourcedocument.Equal(vReqKey))
    ...
    Private Sub assignPurchaseReqToWorkOrderItems(vPurchReqID As Integer)
        ...
        Dim oItems As New WorkorderitemCollection
        'first, pull all work order items that may already be assigned to the select purchase request and unassign them
        oItems.Query.Where(oItems.Query.Workordernumber.Equal(vWorkOrderID), oItems.Query.Sourcetype.Equal("PREQ"), oItems.Query.Sourcedocument.Equal(vPurchReqID))
        If oItems.Query.Load Then
            For Each oitem As Workorderitem In oItems
                oitem.Sourcedocument = Nothing
                oitem.Sourcetype = Nothing
    ...
    Private Sub deleteProdOrder(vProdOrderID As Integer)
        ...
        Dim oItems As New WorkorderitemCollection
        'first, pull all work order items that may already be assigned to the select purchase request and unassign them
        oItems.Query.Where(oItems.Query.Workordernumber.Equal(bsWorkOrder.Current.workordernumber), oItems.Query.Sourcetype.Equal("PORD"), oItems.Query.Sourcedocument.Equal(vProdOrderID))
        If oItems.Query.Load Then
            For Each oitem As Workorderitem In oItems
                oitem.Sourcedocument = Nothing
                oitem.Sourcetype = Nothing
    ...
    Private Sub assignProductionOrderToWorkOrderItems(vOrd As Integer)
        ...
        Dim oItems As New WorkorderitemCollection
        'first, pull all work order items that may already be assigned to the select purchase request and unassign them
        oItems.Query.Where(oItems.Query.Workordernumber.Equal(vWorkOrderID), oItems.Query.Sourcetype.Equal("PORD"), oItems.Query.Sourcedocument.Equal(vOrd))
        If oItems.Query.Load Then
            For Each oitem As Workorderitem In oItems
                oitem.Sourcedocument = Nothing
                oitem.Sourcetype = Nothing
